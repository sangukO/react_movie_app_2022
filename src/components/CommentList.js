...
function CommentList(titleData, id) {
    ...
    const getListData = (dataArry) => {
        if (dataArry === "DeleteData") {
            setComment(
                comment.filter((comment) => data.comment_detail === query)
            );
        } else {
            ...
            arr2.map((data) => {
                ...
                return setComment((comment) => [...comment, value]);
            });
        }
    };
    ...
    return (
        <div>
            ...
            >
                <Table
                    style={{ width: "1010px", paddingTop: "30px" }}
                    dataSource={comment}
                    columns={columns}
                />
            </div>
            ...
            >
                <div>
                    ...
                    <Typography>{/* <pre>{comment_n}번</pre> */}</Typography>
                </div>
            </Modal>
        </div>
    );
